...
function set_history_list()
    ...
    while (end_index>0) do 
        ...
        hiss_timers[end_index] = setTimer(function(account_name,information,amount,state,time,profile_url,element)
            if not panel_state then 
                for k,v in ipairs(history_img_table) do 
                    destroyElement(v)
                end
                for k,v in ipairs(hiss_timers) do 
                    if isTimer(v) then 
                        killTimer(v)
                    end
                end
            ...
            if account_name then 
                for k,v in ipairs(information_img) do 
                    if v[1] == information then 
                        ...
                        if profile_url then 
                            local p = dgsCreateImage(0.06,0.32,0.15,0.55,profile_url,true,element)
                            history[tostring(k).."profile"] = p
                        else
                            local p = dgsCreateImage(0.06,0.32,0.15,0.55,v[3],true,element,v[4])
                            history[tostring(k).."profile"] = p
                        end
                    end
                end
                ...
                for k,v in ipairs(information_img) do 
                    if v[1] == information then 
                        ...
                        dgsSetFont(b,font_h)
                        local i = dgsCreateImage(0.06,0.32,0.15,0.55,v[3],true,element,v[4])
                        history[tostring(k).."label"] = b
                        history[tostring(k).."information"] = i
                    end
                end
            end
            ...
            local circle = dgsCreateImage(0.02,0.2,0.23,0.8,pp_circle,true,element)
            history[tostring(k).."circle"] = circle
            if not panel_state then 
                for k,v in ipairs(history_img_table) do 
                    destroyElement(v)
                end
    ...
    hiss_timers[end_index] = setTimer(function(account_name,information,amount,state,time,profile_url,element)
        ...
        if account_name then 
            for k,v in ipairs(information_img) do 
                if v[1] == information then 
                    if language_section == 1 then 
                        history[tostring(k).."label"] = dgsCreateLabel(0.35,0.42,0.12,0.13,v[2].." "..account_name,true,element,tocolor(255,255,255,205),1,1,50,50,tocolor(1,1,1,255),"left")
                        dgsSetFont( history[tostring(k).."label"],font_h)
                    else
                        history[tostring(k).."label"]= dgsCreateLabel(0.35,0.42,0.12,0.13,account_name.." "..v[2],true,element,tocolor(255,255,255,205),1,1,50,50,tocolor(1,1,1,255),"left")
                        dgsSetFont(history[tostring(k).."label"],font_h)
                    end
                    if profile_url then 
                        history[tostring(k).."profile"] = dgsCreateImage(0.06,0.32,0.15,0.55,profile_url,true,element)
                    else
                        history[tostring(k).."profile"] = dgsCreateImage(0.06,0.32,0.15,0.55,v[3],true,element,v[4])
                    end
                end
            end
        else
            for k,v in ipairs(information_img) do 
                if v[1] == information then 
                    history[tostring(k).."label"] = dgsCreateLabel(0.35,0.42,0.12,0.13,v[2],true,element,tocolor(255,255,255,205),1,1,50,50,tocolor(1,1,1,255),"left")
                    dgsSetFont(history[tostring(k).."label"],font_h)
                    history[tostring(k).."inf"] = dgsCreateImage(0.06,0.32,0.15,0.55,v[3],true,element,v[4])
                end
            end
        end
        history[tostring(k).."time"] = dgsCreateLabel(0.35,0.69,0.12,0.13,time,true,element,tocolor(255,255,255,205),1,1,50,50,tocolor(1,1,1,255),"left")
        dgsSetFont( history[tostring(k).."time"],"arial")
        if state then 
            history[tostring(k).."ucret"]= dgsCreateLabel(0.7,0.65,0.12,0.13,amount..currency,true,element,tocolor(133,187,101,205),1,1,50,50,tocolor(1,1,1,255),"left")
            dgsSetFont(history[tostring(k).."ucret"],font_h)
        else
            history[tostring(k).."ucret"]= dgsCreateLabel(0.7,0.65,0.12,0.13,amount..currency,true,element,tocolor(255,0,0,205),1,1,50,50,tocolor(1,1,1,255),"left")
            dgsSetFont(history[tostring(k).."ucret"] ,font_h)
        end
        local pp_circle = dgsCreateCircle(0.5,0.42,360,tocolor(88,101,242,255))
        history[tostring(k).."circle"] = dgsCreateImage(0.02,0.2,0.23,0.8,pp_circle,true,element)
        if not dgsGetVisible(bank_main_panel) then  
            dgsSetVisible(history[tostring(k).."label"],false)
            dgsSetVisible(history[tostring(k).."profile"],false)
            dgsSetVisible(history[tostring(k).."inf"],false)
            dgsSetVisible(history[tostring(k).."time"],false)
            dgsSetVisible(history[tostring(k).."ucret"],false)
            dgsSetVisible(history[tostring(k).."circle"] ,false)
        end
